...
export function checkHookLiveness(input: LivenessInput): string | null {
  const now = input.now ?? new Date();
  const { start, end } = yesterdayWindow(now);

  const ccYesterday = input.sessions.reduce((n, s) => {
    if (!s.runtime || !s.runtime.startsWith("claude-code")) return n;
    const ts = s.started_at ? Date.parse(s.started_at) : NaN;
    if (!Number.isFinite(ts)) return n;
...
function yesterdayWindow(now: Date): { start: number; end: number } {
