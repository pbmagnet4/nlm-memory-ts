...
export function isLoopbackOrigin(origin, port) {
    if (!origin)
        return false;
    const lower = origin.toLowerCase();
    return (lower === `http://localhost:${port}` ||
...
function registerHermesAgentHookRoutes(app, deps) {
    ...
    app.post("/api/hook/hermes-agent/post-turn", async (c) => {
        ...
        for (const id of surfacedIds) {
            if (assistantResponse.includes(id))
                cited.push(id);
        }
        const preview = assistantResponse.slice(0, 200);
        for (const citedId of cited) {
            await appendCitation({ conversationId: sessionId, citedId, kind: "prose", responsePreview: preview }, ...(deps.citationLogPath !== undefined ? [deps.citationLogPath] : []));
        }
