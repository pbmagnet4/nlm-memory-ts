...
import { tokenSet } from "../recall/tokenize.js";
...
export class FactRecallService {
    ...
    async search(input) {
        ...
        const queryTokens = queryText ? new Set(tokenSet(queryText)) : new Set();
    ...
    async runSemantic(query, byId, fetchLimit) {
        ...
        for (const n of neighbors) {
            ...
            if (!fact)
                continue; // candidate was filtered out by subject/predicate/conf
            hits.push({ fact, similarity: cosineFromL2(n.distance) });
        }
        return hits;
    }
}
function scoreAll(facts, queryTokens) {
    ...
    for (const f of facts) {
        ...
        if (score > 0)
            hits.push({ fact: f, score, matchedIn });
    }
...
function mergeHybrid(kwHits, semHits, byId) {
    ...
    const kwMap = new Map(kwHits.map((h) => [h.fact.id, h]));
    const semMap = new Map(semHits.map((h) => [h.fact.id, h]));
...
function factToHit(fact, score, matchedIn) {
...
function finalize(query, subject, predicate, kind, mode, limit, hits) {
    return {
        query,
        subject,
        predicate,
...
//# sourceMappingURL=fact-recall-service.js.map