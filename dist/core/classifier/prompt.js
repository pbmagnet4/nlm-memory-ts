...
const VOCAB_SET = new Set(PREDICATE_VOCABULARY);
...
export function truncateTranscript(text, maxChars = MAX_TRANSCRIPT_CHARS) {
...
const REQUIRED_KEYS = ["label", "summary", "entities", "decisions", "open", "confidence"];
export function validateClassifierJson(data) {
...
export function buildUserPrompt(transcript, priorContext) {
    const truncated = truncateTranscript(transcript);
    const parts = [];
    if (priorContext)
        parts.push(`PRIOR CONTEXT (already filed):\n${priorContext}\n`);
    parts.push(`TRANSCRIPT TO CLASSIFY:\n${truncated}`);
    return parts.join("\n");
}
function coerceFacts(raw) {
    ...
    for (const item of raw) {
        ...
        const sourceQuote = typeof sourceQuoteRaw === "string" && sourceQuoteRaw.trim().length > 0
            ? sourceQuoteRaw.trim().slice(0, 500)
            : undefined;
        const fact = { kind: kindRaw, subject, predicate, value };
...
export function coerceClassifyResult(data) {
    const strArray = (v) => {
        if (!Array.isArray(v))
            return [];
