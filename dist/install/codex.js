...
export function codexBinaryAvailable() {
 ...
 */
export function writeMcpServerToConfig(configPath) {
...
export function removeMcpServerFromConfig(configPath) {
...
function stripSentinelBlock(content) {
    ...
    if (endIdx < 0)
        return content; // unterminated — refuse to mutate
...
function writeHooksFile(path, content) {
 ...
 */
export function writeLegacyHooks(pluginScriptsDir, hooksPath) {
    ...
    for (const [event, ourGroup] of Object.entries(ourEntries)) {
        ...
        const kept = existing.filter((group) => !group.hooks.some((h) => h.command.includes(LEGACY_HOOK_MARKER)));
        kept.push(ourGroup);
        hooks[event] = kept;
    }
    writeHooksFile(hooksPath, file);
}
export function removeLegacyHooks(hooksPath) {
    ...
    for (const [event, groups] of Object.entries(hooks)) {
        const kept = groups.filter((group) => !group.hooks.some((h) => h.command.includes(LEGACY_HOOK_MARKER)));
...
export function connectCodex(opts, pluginScriptsDir) {
...
export function disconnectCodex(opts) {
    if (opts.dryRun) {
        return {
...
export function pluginScriptsDir(repoRoot) {
